  ...
  const submit = (value: any) => {
    ...
    const pay = {
      ...
      purchaseItems: value.purchaseItems
        ? value.purchaseItems.map((item: any) => {
            return {
              ...item,
              unitPrice: Number(item.unitPrice.replace(/[^0-9-]+/g, "")) / 100,
              amount: Number(item.amount.replace(/[^0-9-]+/g, "")) / 100,
    ...
    <>
      <Space direction="vertical" style={{ width: "100%" }} size="large">
        <Row>
          ...
          <Card>{studentDetails && <BasicDetails data={studentDetails} key={studentDetails?.id} />}</Card>
        </Row>

        <Form
          form={form}
          name="fees-collection-form"
          size="large"
          onFinish={submit}
          initialValues={{
            payType: "FEES",
                            ...
                            <Row justify={"space-between"}>
                              ...
                              <Col span={3} style={{ marginRight: "5vmin" }}>
                                <Form.Item
                                  name={[name, "dueAmount"]}
                                  rules={[
                                    { required: true, message: "Due amount is manditory" },
                                    ({ getFieldValue }) => ({
                                      validator(_, value) {
