import { DeleteTwoTone, ExclamationCircleFilled, PrinterOutlined } from "@ant-design/icons"
import {
  Button,
  Card,
  Col,
...
import {
  ...
  useFetchPaymentDetailsByIdQuery,
} from "../shared/redux/api/feature/payment/api"
...
export const PurchaseSummary = () => {
  ...
  const { data: paySummary } = useFetchPaymentDetailsByIdQuery(id ?? "", { skip: id == undefined })
  ...
  const showDeleteConfirm = (isLineItem: boolean, lineItem: string, itemId: number, purchaseType: string) => {
    confirm({
      title: isLineItem
        ? `Are you sure to delete ${lineItem} from this transaction ?`
      ...
      onOk() {
        isLineItem
          ? onDelete(itemId, purchaseType)
          : deletePayment(itemId).then(message.info("Transaction deleted succesfully."))
      },
    })
  }
  ...
  const onDelete = (lineItemId: number, purchaseType: string) => {
    ...
    }).then((info) => message.info("Updated order succesfully."))
  }
  const feesColumns = [
    {
      title: "Fees Name",
    ...
    return (
      <Table.Summary.Row>
        ...
        <Table.Summary.Cell colSpan={2} index={10}>
          <Text>Approved : {pay.dues.approvedBy.toUpperCase()}</Text>
        </Table.Summary.Cell>
        ...
        <Table.Summary.Cell colSpan={2} index={10}>
          <Tag color={pay.dues.status == "PAID" ? "green" : "red"}>{pay.dues.status}</Tag>
        </Table.Summary.Cell>
      </Table.Summary.Row>
    )
  }
      ...
      {paySummary ? (
        <Space direction="vertical" style={{ width: "100%" }} size="large">
          ...
          <Card key={item?.id}>
            ...
            <Descriptions column={{ xs: 1, sm: 2, md: 3, lg: 3 }}>
              <Descriptions.Item span={1} label="Transaction Id">
                {user?.authority && [Role.ADMIN].includes(user?.authority) && !isMobile ? (
              ...
              <Descriptions.Item span={1} label="Purchase Type">
                <Space style={{ marginTop: "-5px" }}>
                  ...
                  {paySummary?.payType && printReceiptAvailable.includes(paySummary?.payType) && !isMobile && (
              ...
              summary={() => (
                <Table.Summary fixed={"bottom"}>
                  ...
                  <Table.Summary.Row>
                    ...
                    <Table.Summary.Cell colSpan={2} index={1}>
                      Pay Mode :{" "}
                      <Tag color={paySummary?.paymentMode == "CASH" ? "green" : "cyan"}>{paySummary?.paymentMode}</Tag>
                    </Table.Summary.Cell>
